...
async function manuallyDistributeHarvest() {
  const harvestId = process.argv[2] || '11';
  ...
  try {
    ...
    console.log(`\n🚀 Manually distributing harvest #${harvestId}...\n`);
    ...
    if (harvest.rows.length === 0) {
      console.error(`❌ Harvest #${harvestId} not found`);
      process.exit(1);
    }
