...
async function addInvestorProfileTotalWithdrawn() {
  console.log('Adding totalWithdrawn to investor_profiles...\n');
  ...
  try {
    ...
    const columns = await client.execute(`PRAGMA table_info(investor_profiles)`);
    ...
    // Verify
    console.log('\nVerifying...');
    const updatedColumns = await client.execute(`PRAGMA table_info(investor_profiles)`);
...
addInvestorProfileTotalWithdrawn();
