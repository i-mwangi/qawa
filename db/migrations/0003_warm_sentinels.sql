...
CREATE TABLE `__new_coffee_groves` (
	...
	`created_at` integer DEFAULT 1762381560589,
	`updated_at` integer DEFAULT 1762381560589
);
...
CREATE TABLE `__new_environmental_alerts` (
	...
	`created_at` integer DEFAULT 1762381560590,
	`resolved_at` integer,
...
CREATE TABLE `__new_farmer_balances` (
	...
	`updated_at` integer DEFAULT 1762381560591
);
--> statement-breakpoint
...
CREATE TABLE `__new_farmer_withdrawals` (
	...
	`created_at` integer DEFAULT 1762381560590,
	`updated_at` integer DEFAULT 1762381560590,
	FOREIGN KEY (`grove_id`) REFERENCES `coffee_groves`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
CREATE TABLE `__new_investor_verification_history` (
	...
	`verifier_address` text,
	`reason` text,
	`timestamp` integer DEFAULT 1762381560590,
...
CREATE TABLE `__new_liquidity_withdrawals` (
	...
	`requested_at` integer NOT NULL,
	`completed_at` integer,
	`created_at` integer DEFAULT 1762381560591,
	`updated_at` integer DEFAULT 1762381560591
);
--> statement-breakpoint
...
CREATE TABLE `__new_transaction_history` (
	...
	`block_explorer_url` text,
	`metadata` text,
	`created_at` integer DEFAULT 1762381560588,
	`updated_at` integer DEFAULT 1762381560588
);
--> statement-breakpoint
...
CREATE TABLE `__new_farmer_grove_balances` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
	`farmer_address` text NOT NULL,
	`grove_id` integer NOT NULL,
	`this_month_distributed` integer DEFAULT 0 NOT NULL,
	`available_balance` integer DEFAULT 0 NOT NULL,
	`pending_distribution` integer DEFAULT 0 NOT NULL,
	`total_withdrawn` integer DEFAULT 0 NOT NULL,
	`total_earned` integer DEFAULT 0 NOT NULL,
	`last_withdrawal_at` integer,
	`last_calculated_at` integer DEFAULT 1762381560587,
	`updated_at` integer DEFAULT 1762381560587,
	FOREIGN KEY (`grove_id`) REFERENCES `coffee_groves`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
INSERT INTO `__new_farmer_grove_balances`("id", "farmer_address", "grove_id", "this_month_distributed", "available_balance", "pending_distribution", "total_withdrawn", "total_earned", "last_withdrawal_at", "last_calculated_at", "updated_at") SELECT "id", "farmer_address", "grove_id", "this_month_distributed", "available_balance", "pending_distribution", "total_withdrawn", "total_earned", "last_withdrawal_at", "last_calculated_at", "updated_at" FROM `farmer_grove_balances`;--> statement-breakpoint
DROP TABLE `farmer_grove_balances`;--> statement-breakpoint
ALTER TABLE `__new_farmer_grove_balances` RENAME TO `farmer_grove_balances`;--> statement-breakpoint
CREATE INDEX `farmer_grove_balances_farmer_idx` ON `farmer_grove_balances` (`farmer_address`);--> statement-breakpoint
CREATE INDEX `farmer_grove_balances_grove_idx` ON `farmer_grove_balances` (`grove_id`);--> statement-breakpoint
CREATE INDEX `farmer_grove_balances_unique_idx` ON `farmer_grove_balances` (`farmer_address`,`grove_id`);--> statement-breakpoint
CREATE TABLE `__new_farmer_grove_earnings` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
	`farmer_address` text NOT NULL,
	`grove_id` integer NOT NULL,
	...
	`created_at` integer DEFAULT 1762381560585,
	`updated_at` integer DEFAULT 1762381560585,
	FOREIGN KEY (`grove_id`) REFERENCES `coffee_groves`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`harvest_id`) REFERENCES `harvest_records`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
INSERT INTO `__new_farmer_grove_earnings`("id", "farmer_address", "grove_id", "harvest_id", "gross_revenue", "farmer_share", "distribution_status", "distributed_at", "withdrawn_at", "distribution_tx_hash", "withdrawal_tx_hash", "created_at", "updated_at") SELECT "id", "farmer_address", "grove_id", "harvest_id", "gross_revenue", "farmer_share", "distribution_status", "distributed_at", "withdrawn_at", "distribution_tx_hash", "withdrawal_tx_hash", "created_at", "updated_at" FROM `farmer_grove_earnings`;--> statement-breakpoint
DROP TABLE `farmer_grove_earnings`;--> statement-breakpoint
ALTER TABLE `__new_farmer_grove_earnings` RENAME TO `farmer_grove_earnings`;--> statement-breakpoint
CREATE INDEX `farmer_grove_earnings_farmer_idx` ON `farmer_grove_earnings` (`farmer_address`);--> statement-breakpoint
CREATE INDEX `farmer_grove_earnings_grove_idx` ON `farmer_grove_earnings` (`grove_id`);--> statement-breakpoint
CREATE INDEX `farmer_grove_earnings_harvest_idx` ON `farmer_grove_earnings` (`harvest_id`);--> statement-breakpoint
CREATE INDEX `farmer_grove_earnings_status_idx` ON `farmer_grove_earnings` (`distribution_status`);--> statement-breakpoint
CREATE INDEX `farmer_grove_earnings_distributed_idx` ON `farmer_grove_earnings` (`distributed_at`);--> statement-breakpoint
CREATE TABLE `__new_investor_balances` (
	`investor_address` text PRIMARY KEY NOT NULL,
	`total_earnings_all_time` integer DEFAULT 0 NOT NULL,
	`total_earnings_this_month` integer DEFAULT 0 NOT NULL,
	`unclaimed_primary_market` integer DEFAULT 0 NOT NULL,
	`unclaimed_secondary_market` integer DEFAULT 0 NOT NULL,
	`unclaimed_lp_interest` integer DEFAULT 0 NOT NULL,
	`total_unclaimed` integer DEFAULT 0 NOT NULL,
...
--> statement-breakpoint
INSERT INTO `__new_investor_balances`("investor_address", "total_earnings_all_time", "total_earnings_this_month", "unclaimed_primary_market", "unclaimed_secondary_market", "unclaimed_lp_interest", "total_unclaimed", "total_claimed", "last_claim_at", "last_calculated_at", "updated_at") SELECT "investor_address", "total_earnings_all_time", "total_earnings_this_month", "unclaimed_primary_market", "unclaimed_secondary_market", "unclaimed_lp_interest", "total_unclaimed", "total_claimed", "last_claim_at", "last_calculated_at", "updated_at" FROM `investor_balances`;--> statement-breakpoint
DROP TABLE `investor_balances`;--> statement-breakpoint
ALTER TABLE `__new_investor_balances` RENAME TO `investor_balances`;--> statement-breakpoint
CREATE UNIQUE INDEX `investor_balances_investor_address_unique` ON `investor_balances` (`investor_address`);--> statement-breakpoint
CREATE TABLE `__new_investor_claims` (
	`id` text PRIMARY KEY NOT NULL,
	`investor_address` text NOT NULL,
	...
	`requested_at` integer NOT NULL,
	`completed_at` integer,
	`created_at` integer DEFAULT 1762381560587,
	`updated_at` integer DEFAULT 1762381560587
);
--> statement-breakpoint
INSERT INTO `__new_investor_claims`("id", "investor_address", "claim_amount", "earning_ids", "status", "transaction_hash", "block_explorer_url", "error_message", "gas_fee_amount", "requested_at", "completed_at", "created_at", "updated_at") SELECT "id", "investor_address", "claim_amount", "earning_ids", "status", "transaction_hash", "block_explorer_url", "error_message", "gas_fee_amount", "requested_at", "completed_at", "created_at", "updated_at" FROM `investor_claims`;--> statement-breakpoint
DROP TABLE `investor_claims`;--> statement-breakpoint
ALTER TABLE `__new_investor_claims` RENAME TO `investor_claims`;--> statement-breakpoint
CREATE UNIQUE INDEX `investor_claims_id_unique` ON `investor_claims` (`id`);--> statement-breakpoint
CREATE INDEX `investor_claims_investor_idx` ON `investor_claims` (`investor_address`);--> statement-breakpoint
CREATE INDEX `investor_claims_status_idx` ON `investor_claims` (`status`);--> statement-breakpoint
CREATE INDEX `investor_claims_requested_idx` ON `investor_claims` (`requested_at`);--> statement-breakpoint
CREATE TABLE `__new_investor_earnings` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
	`investor_address` text NOT NULL,
	...
	`updated_at` integer DEFAULT 1762381560587,
	FOREIGN KEY (`grove_id`) REFERENCES `coffee_groves`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`holding_id`) REFERENCES `investor_token_holdings`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
INSERT INTO `__new_investor_earnings`("id", "investor_address", "earning_type", "source_id", "grove_id", "holding_id", "token_amount", "earning_amount", "status", "claimed_at", "claim_tx_hash", "distributed_at", "distribution_tx_hash", "created_at", "updated_at") SELECT "id", "investor_address", "earning_type", "source_id", "grove_id", "holding_id", "token_amount", "earning_amount", "status", "claimed_at", "claim_tx_hash", "distributed_at", "distribution_tx_hash", "created_at", "updated_at" FROM `investor_earnings`;--> statement-breakpoint
DROP TABLE `investor_earnings`;--> statement-breakpoint
ALTER TABLE `__new_investor_earnings` RENAME TO `investor_earnings`;--> statement-breakpoint
CREATE INDEX `investor_earnings_investor_idx` ON `investor_earnings` (`investor_address`);--> statement-breakpoint
CREATE INDEX `investor_earnings_type_idx` ON `investor_earnings` (`earning_type`);--> statement-breakpoint
CREATE INDEX `investor_earnings_status_idx` ON `investor_earnings` (`status`);--> statement-breakpoint
CREATE INDEX `investor_earnings_grove_idx` ON `investor_earnings` (`grove_id`);--> statement-breakpoint
CREATE INDEX `investor_earnings_distributed_idx` ON `investor_earnings` (`distributed_at`);--> statement-breakpoint
CREATE INDEX `investor_earnings_claimed_idx` ON `investor_earnings` (`claimed_at`);--> statement-breakpoint
CREATE TABLE `__new_investor_token_holdings` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
	`investor_address` text NOT NULL,
	`grove_id` integer NOT NULL,
	...
	`created_at` integer DEFAULT 1762381560586,
	`updated_at` integer DEFAULT 1762381560586,
	FOREIGN KEY (`grove_id`) REFERENCES `coffee_groves`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
INSERT INTO `__new_investor_token_holdings`("id", "investor_address", "grove_id", "token_amount", "acquisition_type", "purchase_price", "purchase_date", "previous_owner", "transfer_tx_hash", "is_active", "created_at", "updated_at") SELECT "id", "investor_address", "grove_id", "token_amount", "acquisition_type", "purchase_price", "purchase_date", "previous_owner", "transfer_tx_hash", "is_active", "created_at", "updated_at" FROM `investor_token_holdings`;--> statement-breakpoint
DROP TABLE `investor_token_holdings`;--> statement-breakpoint
ALTER TABLE `__new_investor_token_holdings` RENAME TO `investor_token_holdings`;--> statement-breakpoint
CREATE INDEX `investor_token_holdings_investor_idx` ON `investor_token_holdings` (`investor_address`);--> statement-breakpoint
CREATE INDEX `investor_token_holdings_grove_idx` ON `investor_token_holdings` (`grove_id`);--> statement-breakpoint
CREATE INDEX `investor_token_holdings_acquisition_idx` ON `investor_token_holdings` (`acquisition_type`);--> statement-breakpoint
CREATE INDEX `investor_token_holdings_active_idx` ON `investor_token_holdings` (`is_active`);--> statement-breakpoint
CREATE TABLE `__new_lp_token_interest` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
	`provider_address` text NOT NULL,
...
--> statement-breakpoint
INSERT INTO `__new_lp_token_interest`("id", "provider_address", "lp_token_address", "asset_address", "principal_amount", "interest_earned", "interest_rate", "period_start", "period_end", "status", "claimed_at", "claim_tx_hash", "created_at", "updated_at") SELECT "id", "provider_address", "lp_token_address", "asset_address", "principal_amount", "interest_earned", "interest_rate", "period_start", "period_end", "status", "claimed_at", "claim_tx_hash", "created_at", "updated_at" FROM `lp_token_interest`;--> statement-breakpoint
DROP TABLE `lp_token_interest`;--> statement-breakpoint
ALTER TABLE `__new_lp_token_interest` RENAME TO `lp_token_interest`;--> statement-breakpoint
CREATE INDEX `lp_token_interest_provider_idx` ON `lp_token_interest` (`provider_address`);--> statement-breakpoint
CREATE INDEX `lp_token_interest_token_idx` ON `lp_token_interest` (`lp_token_address`);--> statement-breakpoint
CREATE INDEX `lp_token_interest_status_idx` ON `lp_token_interest` (`status`);--> statement-breakpoint
CREATE INDEX `lp_token_interest_period_idx` ON `lp_token_interest` (`period_end`);--> statement-breakpoint
CREATE TABLE `__new_secondary_market_transfers` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
	`grove_id` integer NOT NULL,
	`from_address` text NOT NULL,
	`to_address` text NOT NULL,
...
--> statement-breakpoint
INSERT INTO `__new_secondary_market_transfers`("id", "grove_id", "from_address", "to_address", "token_amount", "transfer_price", "transaction_hash", "status", "transfer_date", "created_at") SELECT "id", "grove_id", "from_address", "to_address", "token_amount", "transfer_price", "transaction_hash", "status", "transfer_date", "created_at" FROM `secondary_market_transfers`;--> statement-breakpoint
DROP TABLE `secondary_market_transfers`;--> statement-breakpoint
ALTER TABLE `__new_secondary_market_transfers` RENAME TO `secondary_market_transfers`;--> statement-breakpoint
CREATE INDEX `secondary_market_transfers_grove_idx` ON `secondary_market_transfers` (`grove_id`);--> statement-breakpoint
CREATE INDEX `secondary_market_transfers_from_idx` ON `secondary_market_transfers` (`from_address`);--> statement-breakpoint
CREATE INDEX `secondary_market_transfers_to_idx` ON `secondary_market_transfers` (`to_address`);--> statement-breakpoint
CREATE INDEX `secondary_market_transfers_date_idx` ON `secondary_market_transfers` (`transfer_date`);